...
Generic two-compartment model for oral dosing with the following features:
	- Option for 2 continuous (AGE, BWT) and 2 categorical variables (SEX, RACE).
		- CL: AGE, SEX, RACE
		- V1: BWT
	- Option for nonlinear clearance
	- Option for bioavability
...
VMAX   :  0    : Maximum velocity (mass/day)
KM     :  2    : Michaelis Constant (mass/L)
TVF1   :  0.5  : Bioavability - EV dose 
...
[ODE]
dxdt_DEPOT  = -KA*DEPOT;
dxdt_CENT   = KA*DEPOT - (K+K12)*CENT - VMAX*C1/(KM+C1) + K21*PERIPH;
